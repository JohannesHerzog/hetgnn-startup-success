#!/usr/bin/env python3
"""Modified Captum IntegratedGradients for heterogeneous graph attribution.

Based on captum.attr.IntegratedGradients (BSD-3-Clause). Modified to support
heterogeneous node feature tensors during gradient computation.
"""
# pyre-strict
import typing
...
def custom_compute_gradients(
    ...
    r"""
    Custom gradient computation to allow unused tensors.
    """
    with torch.autograd.set_grad_enabled(True):
        ...
        assert outputs[0].numel() == 1, (
            ...
            "If you are providing a target index, ensure that the target index "
            "corresponds to a scalar value."
        )
        loss = outputs[0]
        
        # takes gradients
        # Setting allow_unused=True to handle cases where some inputs don't affect output
        grads = torch.autograd.grad(loss, inputs, allow_unused=True) 
        
        # If allow_unused=True, some grads might be None. We should replace them with zeros.
        input_grads = []
