"""Modified Captum CaptumExplainer with support for heterogeneous graph explanations.

Based on PyG's CaptumExplainer (torch_geometric.explain). Modified to handle
HeteroData inputs and produce HeteroExplanation outputs.
"""
import inspect
import logging
import warnings
        ...
        **kwargs,
        ...
        if isinstance(attribution_method, str):
            if attribution_method == "IntegratedGradients":
                self.attribution_method_class = IntegratedGradients
            else:
